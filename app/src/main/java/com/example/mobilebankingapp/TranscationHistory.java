...
public class TranscationHistory extends AppCompatActivity {
    ...
    private void displayDatabaseInfo() {
        ...
        String[] projection = {
                TransactionDone.TransactionEntry.COLUMN_FROM_NAME,
                TransactionDone.TransactionEntry.COLUMN_TO_NAME,
                TransactionDone.TransactionEntry.COLUMN_AMOUNT,
                TransactionDone.TransactionEntry.COLUMN_STATUS
        };

        Log.d("TAG", "displayDataBaseInfo()2");

        Cursor cursor = db.query(
                TransactionDone.TransactionEntry.TABLE_NAME,   // The table to query
        ...
        try {
            ...
            int fromNameColumnIndex = cursor.getColumnIndex(TransactionDone.TransactionEntry.COLUMN_FROM_NAME);
            int ToNameColumnIndex = cursor.getColumnIndex(TransactionDone.TransactionEntry.COLUMN_TO_NAME);
            int amountColumnIndex = cursor.getColumnIndex(TransactionDone.TransactionEntry.COLUMN_AMOUNT);
            int statusColumnIndex = cursor.getColumnIndex(TransactionDone.TransactionEntry.COLUMN_STATUS);
